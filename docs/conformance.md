...
| Link-object `describedby` value forms                                       | supported    | Nested value is the existing sealed `Link`: string link, link object (with its own `href`, `type`, `meta`, `hreflang`, and additional members), or explicit `null`; omission and explicit `null` share the model's absent state because no distinct representation is required; both adapters read and emit recursively |
...
| Unknown unnamespaced members | supported | Readers discard unknown structural members and unrecognized absolute-URI link relations using the bound validation policy while still decoding and validating recognized standard, extension, allowed-profile, and `@` members; absolute-URI link-relation syntax remains supported for retained models and writer validation; aggregate validation of directly constructed models and all writer paths remains strict, and profile members are allowed via context |
...
| Primary-data endpoint role                                                  | supported    | `PrimaryDataContext`: `RESOURCE` is the default ordinary resource context; `RELATIONSHIP` accepts null/single/collection linkage and rejects resource objects with `PRIMARY_DATA_CONTEXT_MISMATCH` at `/data`; a top-level `related` link is accepted only on the relationship endpoint role when primary data is present (explicit null linkage counts as present) while documents without primary data and ordinary resource responses, including related-resource fetches, reject it with `INVALID_LINKS_CONTEXT` at `/links/related`; allowed profile member names do not override this restriction and nested relationship `related` links remain accepted |
...
| Validation context null diagnostics                                         | supported    | `NULL_REQUIRED_VALUE` for usage, endpoint role, policy sets/elements, hints                                                                                                                                                                                                                  |
...
| Rule                                                                                                            | Status       | Notes                                                                                        |
|-----------------------------------------------------------------------------------------------------------------|--------------|----------------------------------------------------------------------------------------------|
| Update primary data must be one resource object (absent, null, collection, or identifier primary data rejected) | supported    | `UPDATE_REQUIRES_SINGLE_RESOURCE` at `/data`; ordinary resource endpoint role only; relationship linkage has no resource-shape requirement |
...
| Every supplied relationship must contain replacement `data`                                                     | supported    | `RELATIONSHIP_DATA_REQUIRED` at `/data/relationships/<name>/data`; primary resource on the resource endpoint only |
...
| Optional expected endpoint identity comparison                                                                  | supported    | `ENDPOINT_IDENTITY_MISMATCH` at `/data/type` or `/data/id`; supplied via `ValidationContext`; primary resource on the resource endpoint only |
| Update rules scoped to the primary resource                                                                     | supported    | `included` resources and relationship linkage keep response semantics; full linkage still enforced |
...
| Rule                                                                                                            | Status       | Notes                                                                                                                     |
|-----------------------------------------------------------------------------------------------------------------|--------------|---------------------------------------------------------------------------------------------------------------------------|
| Create primary data must be one resource object (absent, null, collection, or identifier primary data rejected) | supported    | `CREATE_REQUIRES_SINGLE_RESOURCE` at `/data`; ordinary resource endpoint role only; relationship linkage has no resource-shape requirement |
| Create resource `id` optional; `id` and `lid` remain independent                                                 | supported    | Aggregate validator; primary create resource only; neither substitutes for the other; empty/whitespace strings are present |
| `lid`-only linkage accepted only as a self-reference to the primary create resource                    | supported    | Hosted by the primary resource with matching `type` + `lid`; unrelated linkage, linkage hosted by included resources, and included resources require `id` (`RESOURCE_ID_REQUIRED`) |
| Every relationship supplied on the primary create resource must contain `data`                                  | supported    | Reuses `RELATIONSHIP_DATA_REQUIRED` at `/data/relationships/<name>/data`; primary resource on the resource endpoint only |
| Relationship linkage preserved: null, single, empty and non-empty collection                                    | supported    | All `RelationshipData` variants valid, including matching `type` + `lid` self-reference linkage (unrelated `lid`-only linkage requires `id`) |
...
| Create rules scoped to the primary resource                                                                     | supported    | `included` resources and unrelated relationship linkage are exempt from the primary relationship-data rule and gain no nested-create interpretation; full linkage still enforced; only the primary resource omits `id` and only a matching `type` + `lid` self-reference may omit linkage `id` |
| Links-only/meta-only relationships outside create-specific restrictions                                         | supported    | Valid general core/document representations per ADR-018; rejected only on the primary create resource on the resource endpoint |
...
|---|---|
| Core document model and aggregate validation | `jsonapi-java-core`: `model/JsonApiDocumentSpec`, `model/DocumentDataSpec`, `model/ResourceObjectSpec`, `model/AttributesRelationshipsSpec`, `model/RelationshipSpec`, `model/LinkSpec`, `model/ErrorObjectSpec`, `model/ErrorSourceSpec`, `model/JsonApiObjectSpec`, `aggregate/JsonApiDocumentValidatorSpec`, `validation/MemberNamesSpec`, `internal/SyntaxValidatorsSpec`, `internal/OpenJsonValuesSpec`, `internal/JsonPointersSpec` |
| Create/update request validation | `jsonapi-java-core`: `aggregate/CreateRequestValidationSpec`, `aggregate/UpdateRequestValidationSpec`, `aggregate/JsonApiDocumentValidatorSpec` |
| Links and errors | `jsonapi-java-core`: `model/LinkSpec`, `model/ErrorObjectSpec`, `model/ErrorSourceSpec`, `aggregate/JsonApiDocumentValidatorSpec`; both adapters: `DocumentReaderSpec`, `DocumentWriterContractSpec`, `internal/JsonApiWireWriterSpec` |
...
| Unknown relationship cardinality is permissive for pagination | `jsonapi-java-core`: `aggregate/JsonApiDocumentValidatorSpec` (absent linkage with no hint allowed; `TO_MANY` allowed; null/single linkage and `TO_ONE` hint rejected) |
...
| Read-tolerant/write-strict unknown members | both adapters: `DocumentReaderSpec` (unknown structural members discarded; deeper identifier/jsonapi/error/source and link-object branches; recognized malformed link value still fails; extension/profile/`@` retained; duplicate unknowns tolerated while retained duplicates fail); `jsonapi-java-jackson-api`: `internal/JacksonInternalHelpersSpec` (retained structural and recognized link classification); core validation specs unchanged as proof that direct construction and writers still reject unsupported members |
