...
| Primary-data endpoint role                                                  | supported    | `PrimaryDataContext`: `RESOURCE` is the default ordinary resource context; `RELATIONSHIP` accepts null/single/collection linkage and rejects resource objects with `PRIMARY_DATA_CONTEXT_MISMATCH` at `/data`; endpoint context is represented while selective top-level `related` handling remains pending |
