...
- `jsonapi-java-mapping` owns the internal cross-artifact mapping implementation namespace
  consumed by backend runtimes, including backend-neutral compound-inclusion traversal, inclusion
  state, and basic resource-write orchestration. It depends on the neutral API and is published on
  the unified release train, but it is unsupported consumer API.
