 ...
 * additionally owns the advanced relationship and meta phase: relationship cardinality, null/empty
 * short-circuiting, direct {@link com.kazforge.jsonapi.core.model.ResourceIdentifier} copies that
 * preserve identifier meta and drop additional members, opt-in {@code RelationshipLinkage}
 * occurrence orchestration, and resource/relationship meta presence and raw-member binding. The
 * relationship-linkage orchestration is shared with low-level PATCH binding through {@link
 * RelationshipLinkageBinder}; this reader owns the read phase order around it. The resource-type
 * authority stays the existing {@link ResourceTypeMatch}.
 *
...
public final class BasicResourceReader<T, P> {
  ...
  private final RelationshipLinkageBinder<T, ReadProperty<P>> linkageBinder;
  ...
  private void bindRelationships(
    ...
    for (ReadProperty<P> property : definition.relationships()) {
      ...
      properties.put(property.externalName(), linkageBinder.bind(property, data));
    }
  }
