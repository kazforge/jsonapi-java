...
class JsonApiDraftSchemaSpec extends Specification {
  ...
  def "writer-produced resource response validates against the draft response schema"() {
    given:
    def json = JsonApiJackson2.jsonApi(mapper).resources().writeOne(
        new LocalIdentityArticle("1", null, "Title"))

    expect:
    schemas["response"].validate(json, InputFormat.JSON).isEmpty()
  }

  def "writer-produced create document validates against the draft create schema"() {
    given:
    def json = JsonApiJackson2.jsonApi(mapper).resources().writeCreateDocument(
        new LocalIdentityArticle(null, "temp-1", "Draft"))

    expect:
    schemas["create"].validate(json, InputFormat.JSON).isEmpty()
  }
