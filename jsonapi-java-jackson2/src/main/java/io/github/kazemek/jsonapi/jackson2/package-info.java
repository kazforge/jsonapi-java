 ...
 * reading, and advanced annotated-domain-to-resource mapping.
 *
 ...
 * {@link JsonApiJackson2#resourceMapper} as the sole public mapping path; {@link
 ...
 * explicit handoff to the writer.
 *
 * <p>Additional capabilities (flat binding, presence-aware PATCH, and the Level-1 configured
 * runtime) follow in later parity stories; this package holds the validated document-output and
 * document-read contracts plus the advanced write-side domain mapping. Cross-major parity is
 ...
 * {@code reader(mapper, DocumentReadContext)}, and {@code resourceMapper(mapper,
 * identifierConverter, decoratorRegistry)} with meaningful default conveniences for the writer and
 * resource mapper. {@code JsonMapper.Builder} overloads are intentionally not part of the public
 * contract. The writer derives an isolated codec mapper via {@code rebuild()}; the reader uses the
 * supplied mapper directly for token-driven parsing; the resource mapper derives an isolated
 * mapping mapper via {@code rebuild()} with only mapping-required internal module support,
 * including a caller-preserving JDK 8 {@code Optional} fallback. No construction path mutates the
 ...
 * JsonApiDocumentReadException} with rule codes on reads; mapping failures throw {@link
 ...
 * registries, and provenance values are Jackson-major-neutral contracts in {@link
 * io.github.kazemek.jsonapi.jackson}; this package holds only the Jackson 2-bound writer, reader,
 * resource mapper, and their implementations.
 */
@NullMarked
