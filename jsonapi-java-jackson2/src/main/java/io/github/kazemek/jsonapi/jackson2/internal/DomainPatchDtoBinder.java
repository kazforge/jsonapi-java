 ...
 * modules remain authoritative. The JSON:API identifier is parsed first and then follows the same
...
public final class DomainPatchDtoBinder {
  ...
  private void validatePatchableMetaProperty(
    ...
    if (!validTypedPatchTarget(property.definition().getPrimaryType())) {
      throw new JsonApiMappingException(
