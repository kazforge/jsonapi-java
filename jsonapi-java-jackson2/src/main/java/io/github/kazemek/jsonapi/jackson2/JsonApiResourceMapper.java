...
import io.github.kazemek.jsonapi.jackson2.internal.CompoundInclusionEngine;
import io.github.kazemek.jsonapi.jackson2.internal.DomainResourceWriter;
import io.github.kazemek.jsonapi.jackson2.internal.EffectiveRepresentation;
import io.github.kazemek.jsonapi.jackson2.internal.IncludedResourcesResult;
 ...
 * JsonApiJackson2#resourceMapper(com.fasterxml.jackson.databind.json.JsonMapper)} or its overloads,
 * never directly. The mapper is safe for concurrent use once created. Decorators supplied through a
 ...
 * definitions by complete declared type on this mapper instance. JSON:API annotations assign
 * semantic roles; configured Jackson owns property discovery, visibility, external naming, mix-ins,
 * creators, and value conversion. Unannotated Jackson-visible properties do not participate, except
 * the conventional identifier whose Jackson external name is {@code id}. {@code @JsonApiId} maps
 * only {@link ResourceObject#id()} and {@code @JsonApiLocalId} maps only {@link
 * ResourceObject#lid()}; the two identity roles never fall back to each other.
 *
 * <p>Mapping is write-only: this mapper produces core model objects. Feed them to a {@link
 * JsonApiDocumentWriter} for serialization.
 *
