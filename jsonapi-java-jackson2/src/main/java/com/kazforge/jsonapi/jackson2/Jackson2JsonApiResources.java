...
import com.kazforge.jsonapi.jackson.api.ResourceCollectionDocument;
import com.kazforge.jsonapi.jackson.api.ResourceDocument;
...
/**
 * Jackson 2 Level-1 resource operations: strict homogeneous reads, single/collection writes, and
 ...
 * decoration, validate the mapped document, and emit without caller choreography. Checked Jackson 2
 * I/O is exposed as {@link java.io.UncheckedIOException} at this Level-1 boundary.
 */
final class Jackson2JsonApiResources implements JsonApiResources {
  ...
  public <T> T readOne(String json, Class<T> type) {
    ...
    return type.cast(bindSingle(Jackson2Io.call(() -> resourceReader.readValue(json)), type));
  }
  ...
  public Object readOne(String json, Type type) {
    ...
    Objects.requireNonNull(type, "type");
    return bindSingle(
        Jackson2Io.call(() -> resourceReader.readValue(json)), baseMapper.constructType(type));
  }
  ...
  public <T> List<T> readMany(String json, Class<T> type) {
    ...
    return bindCollection(
        Jackson2Io.call(() -> resourceReader.readValue(json)),
        baseMapper.constructType(type),
        type);
  }
  ...
  public <T> ResourceDocument<T> readOneDocument(String json, Class<T> type) {
    Objects.requireNonNull(json, "json");
    Objects.requireNonNull(type, "type");
    return documentSingle(Jackson2Io.call(() -> resourceReader.readValue(json)), type);
  }
  ...
  public <T> ResourceCollectionDocument<T> readManyDocument(String json, Class<T> type) {
    Objects.requireNonNull(json, "json");
    Objects.requireNonNull(type, "type");
    return documentCollection(Jackson2Io.call(() -> resourceReader.readValue(json)), type);
  }
  ...
  public String writeOne(Object resource, ResourceWriteOptions options) {
    ...
    return Jackson2Io.call(
        () -> responseWriter.writeValueAsString(mappedSingle(resource, options)));
  }
  ...
  public void writeOne(Object resource, ResourceWriteOptions options, OutputStream out) {
    ...
    Jackson2Io.run(() -> responseWriter.writeValue(out, mappedSingle(resource, options)));
  }
  ...
  public String writeMany(Iterable<?> resources, ResourceWriteOptions options) {
    ...
    Objects.requireNonNull(options, OPTIONS);
    return Jackson2Io.call(
        () -> responseWriter.writeValueAsString(mappedCollection(resources, options)));
  }
  ...
  public void writeMany(Iterable<?> resources, ResourceWriteOptions options, OutputStream out) {
    ...
    Objects.requireNonNull(options, OPTIONS);
    Objects.requireNonNull(out, "out");
    Jackson2Io.run(() -> responseWriter.writeValue(out, mappedCollection(resources, options)));
  }
  ...
  public String writeCreateDocument(Object resource, ResourceWriteOptions options) {
    Objects.requireNonNull(resource, RESOURCE);
    Objects.requireNonNull(options, OPTIONS);
    return Jackson2Io.call(() -> createWriter.writeValueAsString(mappedCreate(resource, options)));
  }
  ...
  public void writeCreateDocument(Object resource, ResourceWriteOptions options, OutputStream out) {
    ...
    Objects.requireNonNull(options, OPTIONS);
    Objects.requireNonNull(out, "out");
    Jackson2Io.run(() -> createWriter.writeValue(out, mappedCreate(resource, options)));
  }
  ...
  public String writeUpdateDocument(
    ...
    Objects.requireNonNull(options, OPTIONS);
    return Jackson2Io.call(
        () -> updateWriter(expectedIdentity).writeValueAsString(mappedSingle(resource, options)));
  }
  ...
  private Object bindSingle(JsonApiDocument document, Class<?> type) {
    ...
    return resourceBinder.fromResource(resource, type);
  }
  ...
  private <T> ResourceDocument<T> documentSingle(JsonApiDocument document, Class<T> type) {
    ResourceObject resource = requireSingleResource(document, type);
    T bound = type.cast(resourceBinder.fromResource(resource, type));
