 ...
 * sealed variants such as {@link com.kazforge.jsonapi.core.model.DocumentData.NullData}. Use {@link
 * JsonApiJackson2#writer} as the sole public writer path; the writer validates before emission, so
 * validation failure cannot leave a partially written document. Use {@link JsonApiJackson2#reader}
 * as the sole public reader path; the reader decodes token-driven wire JSON through public core
 * constructors, then runs aggregate validation before returning a document. Use {@link
 * JsonApiJackson2#resourceMapper} as the sole public write-mapping path; {@link
 * JsonApiResourceMapper} maps annotated domain values to core {@link
 * com.kazforge.jsonapi.core.model.ResourceObject} and {@link
 * com.kazforge.jsonapi.core.model.JsonApiDocument} values, keeping serialization as an explicit
 * handoff to the writer. Use {@link JsonApiJackson2#resourceBinder} as the sole public flat-binding
 * path; {@link JsonApiResourceBinder} binds already-validated {@link
 * com.kazforge.jsonapi.core.model.ResourceObject} values to annotated flat DTO types without
 ...
 * com.kazforge.jsonapi.jackson.patch.PatchCommand} or directly into an annotated {@link
 * com.kazforge.jsonapi.jackson.patch.PatchPresence} DTO.
 *
 ...
 * payload-safe {@link com.kazforge.jsonapi.jackson.diagnostic.JsonApiDocumentReadException} values;
 * core validation failures stay unchecked {@link
 * com.kazforge.jsonapi.core.validation.JsonApiValidationException} on writes and become {@code
 * JsonApiDocumentReadException} with rule codes on reads; mapping and binding failures throw {@link
 * com.kazforge.jsonapi.jackson.diagnostic.JsonApiMappingException} with {@link
 * com.kazforge.jsonapi.jackson.diagnostic.MappingDiagnostic} values.
 *
 * <p>Writing a {@link com.kazforge.jsonapi.jackson.mapping.MappedDocument} is provenance aware: the
 * writer composes its bound validation context with the mapping's sparse-fieldset linkage
 * exemptions before validating, so callers never translate mapping provenance into validation
 * policy themselves.
 *
 * <p>Use {@link JsonApiJackson2#domainDocumentReader} for the advanced heterogeneous path. It
 * consumes the Jackson-neutral {@link com.kazforge.jsonapi.jackson.mapping.ResourceTypeRegistry}
 * explicitly, decodes and validates through {@link JsonApiDocumentReader}, binds primary and {@code
 * included} resources independently, and preserves document-level members. The resulting {@link
 * JsonApiDomainDocument} keeps {@code included} DTOs ordered and identity-indexed without injecting
 * them into linkage relationships; identifier primary data remains core identifier data. Its {@code
 * metaAs} methods use the reader-derived configured mapper. This registry-backed path is advanced
 * and does not alter the registry-free, homogeneous Level-1 resource reads.
 *
 ...
 * wrapper are Jackson-major-neutral contracts in {@link com.kazforge.jsonapi.jackson}; this package
 * holds the Jackson 2-bound Level-1 runtime, writer, reader, resource mapper, resource binder,
 * typed domain envelope, PATCH readers, and their implementations. Level-1 adapts unavoidable
 * Jackson 2 checked stream I/O to {@link java.io.UncheckedIOException}; existing document-read,
 * validation, and mapping exception families remain distinct.
 */
@NullMarked
