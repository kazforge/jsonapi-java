 ...
 * deep construction-failure pointer translation. Each nested shape isolates one declaration or
 * diagnostic mechanic.
 */
...
public final class PatchStructureFixtures {
   ...
   * logicalName} divergence is preserved in the {@link com.kazforge.jsonapi.patch.StructuredPatch}.
   */
   ...
   * naming strategy applies to nested structured marker maps.
   */
   ...
   * member, proving nested wrapper customization is rejected on shape entry.
   */
   ...
   * and rejected on typed shape entry.
   */
   ...
   * shape entry. Never actually invoked: the member is rejected during declaration validation.
   */
   ...
   * symmetrically and rejected on typed shape entry.
   */
   ...
   * customization on a creator parameter is detected and rejected on typed shape entry.
   */
   ...
   * failure with no property path so the shape-translated pointer falls back to the root.
   */
  @JsonApiResource(type = "articles")
