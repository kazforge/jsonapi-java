...
import com.kazforge.jsonapi.jackson3.internal.Jackson3PrototypeMappingBackend
import spock.lang.Specification
import tools.jackson.databind.json.JsonMapper

class Jackson3MappingBackendContractSpec extends Specification {
  ...
  def "shared writer keeps JSON library external names separate from JSON API identity members"() {
    ...
    def writer = new GenericDomainResourceWriter<>(
        new Jackson3PrototypeMappingBackend(JsonMapper.builder().build()))
  ...
  def "shared mapping-domain writer satisfies the same contract"() {
    ...
    def writer = new GenericDomainResourceWriter<>(new Jackson3PrototypeMappingBackend(base))
    def adapter = new MappingContractAdapter() {
          @Override
          ResourceObject toResource(Object resource) {
